...
const InstIFPipeline = () => {
  ...
  const hazardMessage = pipelineValuesStages.IF.HazardMessage
    ? (pipelineValuesStages.IF.HazardMessage as string).split(" | ").join("\n\n")
    : "";
  ...
  return (
    <div className="relative flex items-center gap-5 ml-1 h-full bg-[#66939E] px-[1.2rem] py-[.7rem] rounded-[.6rem] text-white max-w-max">
      ...
      {pipelineValuesStages.IF.instruction.pc !== -1 && (
        <p className="text-[1.6rem]">
          PC: <span className="text-[1.8rem]">{pipelineValuesStages.IF.instruction.inst}</span>
        </p>
      )}

      {pipelineValuesStages.IF.HazardMessage && (
