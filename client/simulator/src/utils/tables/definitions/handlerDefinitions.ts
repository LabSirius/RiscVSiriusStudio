...
export const valueFormatter = (cell: CellComponent) => {
  ...
  switch (viewType) {
    ...
    case 16:
      return binaryToHex(value).toUpperCase();
    case "ascii":
...
const isValidAs = (value: string, valType: RegisterView) => {
  switch (valType) {
    case 2:
      return validBinary(value) && value.length === 32;
    case "unsigned":
  ...
  const keyDownHandler = (evt: KeyboardEvent) => {
    ...
    switch (key) {
      ...
      case "s": {
        ...
        newContent =
          unsignedVal.toString() === signedVal.toString()
            ? signedVal.toString()
...
export const createTooltip = (
  ...
  tooltip.className =
    " text-white rounded-md px-2 py-1 overflow-hidden shadow-md max-w-min h-[1.8rem] z-[99999]";
...
export const attachMemoryConversionToggle = (cell: CellComponent): void => {
  ...
  const handleKeyDown = (evt: KeyboardEvent): void => {
    ...
    if (isAddressField) {
      ...
      switch (key) {
        case "b":
          newContent = cellValue.padStart(8, "0");
          activeKey = "b";
          break;
        case "h":
          ...
          break;
        case "s": {
          const signedVal = binaryToInt(cellValue);
          const unsignedVal = binaryToUInt(cellValue);
          newContent =
            signedVal.toString() === unsignedVal.toString()
              ? signedVal.toString()
              : `${signedVal} / ${unsignedVal}`;
          activeKey = "s";
          break;
        ...
        case "u":
          newContent = binaryToUInt(cellValue).toString();
          activeKey = "u";
          break;
        case "a":
          try {
            const decimal = parseInt(cellValue, 2);
            const char = String.fromCharCode(decimal);
            newContent = /^[\x20-\x7E]$/.test(char) ? `'${char}'` : " ";
            activeKey = "a";
          } catch {
            newContent = " ";
            activeKey = "a";
